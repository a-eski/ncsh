...
# Setup
def setup_tests(prompt_directory_option)
  @directory = Dir.pwd
  @user = ENV['USER']
  case prompt_directory_option
  when PROMPT_DIRECTORY_NORMAL
    @start_column = @user.length + 1 + @directory.length + PROMPT_LENGTH
  when PROMPT_DIRECTORY_SHORT
    @start_column = @user.length + 1 + @directory.length - HOME_LENGTH + PROMPT_LENGTH
  when PROMPT_DIRECTORY_NONE
    @start_column = @user.length + PROMPT_LENGTH
  end
