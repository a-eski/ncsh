...
eskilib_nodiscard int_fast32_t ncsh_vm_execute_external(char** buffer,
                                                        struct ncsh_Tokens* tokens,
                                                        struct ncsh_Vm* vm,
                                                        uint_fast32_t command_position,
                                                        uint_fast8_t op_current,
                                                        __pid_t* pid)
{
    ...
    *pid = fork();
    ...
    if (*pid == 0) {
        ...
        if (execvp(buffer[0], buffer) == -1) {
            ...
            fflush(stdout);
            kill(getpid(), SIGTERM);
            return NCSH_COMMAND_EXECVP_FAILED;
        }
    }
...
eskilib_nodiscard int_fast32_t ncsh_vm_execute_external_result(pid_t pid)
{
    int status = 0;
...
eskilib_nodiscard int_fast32_t ncsh_vm(struct ncsh_Args* args)
{
    ...
    pid_t pid = 0;
    int status;
    int command_result = NCSH_COMMAND_NONE; // result of execvp or builtin command.
    struct ncsh_Vm vm = {0};
    char* buffer[MAX_INPUT] = {0};
    bool end = false;
    enum ncsh_Ops op_current = OP_NONE;
    enum ncsh_Command_Type command_type_current = CT_NONE;
    ...
    while (args->values[args_position] != NULL && end != true) {
        ...
        // Check if we are using a builtin
        if (args_position == 0) {
            for (uint_fast32_t i = 0; i < sizeof(builtins) / sizeof(char*); ++i) {
                if (eskilib_string_compare(args->values[0], args->lengths[0], builtins[i], builtins_len[i])) {
                    ...
                    command_type_current = CT_BUILTIN;
        ...
        if (command_result == NCSH_COMMAND_NONE) {
            command_type_current = CT_EXTERNAL;
            command_result = ncsh_vm_execute_external(buffer, &tokens, &vm, command_position, op_current, &pid);
        }
        ...
        if (command_result == NCSH_COMMAND_EXECVP_FAILED) {
            break;
        }

        if (command_type_current == CT_EXTERNAL) {
            ncsh_vm_atomic_child_pid_set(pid);
        ...
        ++command_position;
        command_result = NCSH_COMMAND_NONE;
    }
...
eskilib_nodiscard int_fast32_t ncsh_vm_execute(struct ncsh_Shell* shell)
{
    ...
    // z is not subject to redirection and other operations.
