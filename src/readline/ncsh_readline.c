                           ...
                           struct ncsh_Arena* const scratch_arena)
{
    ...
    while (1) {
        ...
        case TAB_KEY: {
	    if ((exit = ncsh_readline_tab_autocomplete(input, scratch_arena)) != EXIT_SUCCESS) {
		goto exit;
	    }
        ...
        default: {
            ...
            exit = ncsh_readline_putchar(character, input);
            switch (exit) {
            case EXIT_CONTINUE: {
                continue;
            }
        ...
        if (sigwinch_caught) {
            if (ncsh_readline_resize(input) != EXIT_SUCCESS) {
                exit = EXIT_FAILURE;
                break;
            }
            sigwinch_caught = 0;
