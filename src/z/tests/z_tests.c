...
void z_read_from_database_file_empty_database_file_test(void) {
	...
	eskilib_assert(z_init(&db) == Z_SUCCESS);
...
void z_add_to_database_empty_database_test(void) {
	...
	eskilib_assert(add_to_database(new_value.value, new_value.length, &db) == Z_SUCCESS);
...
void z_write_to_database_file_empty_database_test(void) {
	...
	eskilib_assert(write_to_database_file(db.count, db.dirs) == Z_SUCCESS);
...
void z_add_to_database_empty_value_test(void) {
	...
	eskilib_assert(add_to_database(eskilib_String_Empty.value, eskilib_String_Empty.length, &db) == Z_NULL_REFERENCE);
	eskilib_assert(db.count == 1);
...
void z_home_change_directory_test() {
	struct z_Database db = {0};
	eskilib_assert(z_init(&db) == Z_SUCCESS);
	eskilib_assert(db.count == 2);
