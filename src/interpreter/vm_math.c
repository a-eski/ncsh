...
Str vm_math_expr(Vm_Data* restrict vm)
{
    ...
    if (vm->ops[0] != OP_MATH_EXPR_START || vm->ops[1] != OP_NUM) {
        tty_fputs("ncsh: unable to process math expression.", stderr);
        return Str_Empty;
    }

    // Setup
    Expr* exs = arena_malloc(vm->s, vm->strs_n, Expr);
    exs->n = vm->strs_n;
    for (size_t i = 1; i < exs->n; ++i) {
        if (vm->ops[i] == OP_NUM) {
            exs[i] = (Expr){.type = M_NUM, .val.num = estrtonum(vm->strs[i])};
            continue;
        }
        if (vm->ops[i] == OP_MATH_EXPR_END)
            break;
        exs[i] = (Expr){.type = M_OP, .val.op = vm->ops[i]};
    }
