...
void vm_buffer_set_command_next(Statements* rst stmts, Vm_Data* rst vm)
{
    if (!stmts) {
        vm->buffer[0] = NULL;
        vm->end = true;
    ...
    size_t vm_buf_pos = 0;
    while (stmts && stmts->val) {
        if (stmts->op == OP_PIPE || stmts->op == OP_AND || stmts->op == OP_OR)
            break;
        assert(stmts->val[stmts->len - 1] == '\0');
        vm->buffer[vm_buf_pos] = stmts->val;
        vm->buffer_lens[vm_buf_pos] = stmts->len;
        debugf("set vm->buffer[%zu] to %s, %zu\n", vm_buf_pos, tok->val, tok->len);

        if (!stmts->next || !stmts->next->val) {
            vm->end = true;
            ++vm_buf_pos;
            break;
        }
...
[[nodiscard]]
void vm_buffer_set(Statements* rst stmts, Vm_Data* rst vm)
{
