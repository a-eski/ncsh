...
#define eskilib_assert(condition) if (!(condition)) { printf("%s condition failed on line %d\n", __func__, __LINE__); eskilib_test_failed_internal(); return; }

extern void eskilib_test_run(const char* function_name, void (*function)(void));

// private API: do not need to use this manually, used with eskilib_assert to mark test failed when assertion does not pass.
